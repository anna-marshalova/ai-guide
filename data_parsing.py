...
class WikiParser:
    def __init__(self, base_url="https://ru.wikipedia.org"):
        ...
        self.wiki_html = wikipediaapi.Wikipedia('MyProjectName (merlin@example.com)', 'ru', extract_format=wikipediaapi.ExtractFormat.HTML)
        ...
        def get_categorymembers(members, result={}, level=0, max_level=1):
            ...
            for member in pbar:
                ...
                if member.ns == wikipediaapi.Namespace.CATEGORY and level < max_level:
                    result.update(get_categorymembers(member.categorymembers, result=result, level=level + 1, max_level=max_level))
            return result
    ...
    def validate_section_title(self, section_title):
        if section_title.get('class') in ['mw-toc-heading', 'vector-menu-heading']:
            return False
        if section_title.get('id') in ['mw-toc-heading', 'Ссылки', 'Литература', 'Примечания', 'См. также', 'Навигация', 'Прочее']:
            return False
        ...
        try:
            ...
            for section_div in section_divs:
                ...
                if self.validate_section_title(section) and title:
                    section_content = {title: []}
                    current = section_div.find_next_sibling()
                    while current and current.name not in ['h2'] and set(current.get('class', []))&{'mw-heading2'} == set():
                        if self.validate_content(current):
                            section_content[title].append(current.get_text().strip())
...
def get_data(big_city_limit=None, russian_city_limit=None, tourism_limit=None):
    ...
    russian_cities_pages = wikipedia_parser.get_pages_from_table("Список городов России", "Город")
    big_cities_pages = wikipedia_parser.get_pages_from_table("Список городов с населением более миллиона человек", "Город")
    tourism_pages = wikipedia_parser.get_pages_from_category("Категория:Туризм по странам", max_pages=tourism_limit)
    
    russian_cities_data = wikipedia_parser.scrape_pages(russian_cities_pages, russian_city_limit)
    ...
    wikivoyage_russian_cities_data = wikivoyage_parser.scrape_pages(russian_cities_pages, russian_city_limit)
    wikivoyage_big_cities_data = wikivoyage_parser.scrape_pages(big_cities_pages, big_city_limit)
            
    return russian_cities_data, big_cities_data, tourism_data, wikivoyage_russian_cities_data, wikivoyage_big_cities_data
...
def main(big_city_limit=None, russian_city_limit=None, tourism_limit=None):
    russian_cities_data, big_cities_data, tourism_data, wikivoyage_russian_cities_data, wikivoyage_big_cities_data = get_data(big_city_limit, russian_city_limit, tourism_limit)
